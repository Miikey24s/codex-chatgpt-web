...
test("tool_choice constraints are preserved in the browser transport contract", () => {
  const token = "turn_12345678901234567890123456789012";
  const capabilities = { localToolsEnabled: true, solAvailable: true, extraHighAvailable: true, proAvailable: true };

  const forced = request("high");
  forced.options.toolChoice = { name: "read_file" };
  const forcedPrompt = compileChatGptWebPrompt(forced, capabilities, token).text;
  expect(forcedPrompt).toContain('requires the outer Responses tool "read_file"');
  expect(forcedPrompt).toContain('codex_tool_call with wire_name="read_file"');

  const none = request("high");
  none.options.toolChoice = "none";
  expect(compileChatGptWebPrompt(none, capabilities, token).text)
    .toContain("tool_choice to none. Do not call any Codex Native tool");

  const allowed = request("high");
  allowed.options.toolChoice = { allowedTools: ["read_file", "list_files"], mode: "required" };
  const allowedPrompt = compileChatGptWebPrompt(allowed, capabilities, token).text;
  expect(allowedPrompt).toContain('allowed set: "read_file", "list_files"');
  expect(allowedPrompt).toContain("At least one outer tool from that allowed set must complete before a final answer.");
});
